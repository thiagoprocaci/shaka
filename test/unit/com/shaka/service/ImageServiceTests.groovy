...
class ImageServiceTests extends GrailsUnitTestCase {
    ...
    void testSaveImageSucesso(){
        try {
            MultipartFile multipartFile = getMockMultipartFile(getAbsolutePath("/com/shaka/resources/shaka.jpg"));
            assertNotNull multipartFile
            imageService.saveImage(getAbsolutePath("/com/shaka/resources/"), "shaka_test.jpg", multipartFile)
            // verifica se o arquivo foi salvo corretamente
            File file = new File(getAbsolutePath("/com/shaka/resources/shaka_test.jpg"))
            assertNotNull file
    ...
    void testSaveImageDiretorioNaoExistente(){
        try {
            MultipartFile multipartFile = getMockMultipartFile(getAbsolutePath("/com/shaka/resources/shaka.jpg"));
            assertNotNull multipartFile
            imageService.saveImage("diretorioNaoExistente", "shaka_test.jpg", multipartFile)
        } catch (IOException e) {
            assertNotNull e
        }
    }
    ...
    void testDeleteImageDiretorioNaoExistente(){
        try {
            imageService.deleteImage("diretorioNaoExistente", "shaka_test.jpg")
        } catch (IOException e) {
            assertNotNull e
        }
    }
    ...
    void testDeleteImageSucesso(){
        try {
            MultipartFile multipartFile = getMockMultipartFile(getAbsolutePath("/com/shaka/resources/shaka.jpg"));
            assertNotNull multipartFile
            multipartFile.transferTo(new File(getAbsolutePath("/com/shaka/resources/") + "shaka_test.jpg"))
            imageService.deleteImage(getAbsolutePath("/com/shaka/resources/"), "shaka_test.jpg")
        } catch (IOException e) {
            fail()
        }
    }
